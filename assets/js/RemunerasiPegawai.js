...
$(document).ready(function(){ 
    ...
    $('#add').click(function(){
     ...
     var error_tgl_gaji = '';
       if($('#tgl_gaji').val() == '')
       {
         ...
         $('#perkiraan_id').val('');
         $('#jumlah_remunerasi').val('');
         $('#error_perkiraan_id').text('');
         $('#error_jumlah_remunerasi').text('');
         $('#perkiraan_id').css('border-color', '');
         $('#jumlah_remunerasi').css('border-color', '');
         $('#save').text('Save');
    ...
    $('#save').click(function(){
     ...
     var error_jumlah_remunerasi = '';
     var perkiraan_id = '';
     var jumlah_remunerasi = '';
     if($('#perkiraan_id').val() == '')
     {
     ...
     if($('#jumlah_remunerasi').val() == '')
     {
      error_jumlah_remunerasi = 'Jumlah remunerasi perlu diisi';
      $('#error_jumlah_remunerasi').text(error_jumlah_remunerasi);
      $('#jumlah_remunerasi').css('border-color', '#cc0000');
      jumlah_remunerasi = '';
     }
     else
     {
      error_jumlah_remunerasi = '';
      $('#error_jumlah_remunerasi').text(error_jumlah_remunerasi);
      $('#jumlah_remunerasi').css('border-color', '');
      jumlah_remunerasi = $('#jumlah_remunerasi').val();
     }
     if(error_perkiraan_id != '' || error_jumlah_remunerasi != '')
     {
      return false;
     }
     else
     {
      if($('#save').text() == 'Save')
      {
       ...
       output += '<td>'+jumlah_remunerasi+'<input type="hidden" id="jumlah_remunerasi'+count+'" name="hidden_jumlah_remunerasi[]"  value="'+jumlah_remunerasi+'" /></td>';
      ...
      else 
      {
       ...
       output += '<td>'+jumlah_remunerasi+' <input type="hidden" name="hidden_jumlah_remunerasi[]" id="jumlah_remunerasi'+row_id+'" value="'+jumlah_remunerasi+'" /></td>';
    ...
    $(document).on('click', '.view_details', function(){
     ...
     var jumlah_remunerasi = $('#jumlah_remunerasi'+row_id+'').val();
     $('#perkiraan_id').val(perkiraan_id);
     $('#jumlah_remunerasi').val(jumlah_remunerasi);
     $('#save').text('Edit');
   ...
   $('#perkiraan_form').on('submit', function(event){
     ...
     if(count_data > 0)
     {
      ...
      $.ajax({
       url:"Remunerasi/tambahRemunerasi",
       method:"POST",
       data:form_data,
