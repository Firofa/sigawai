...
class Umakan extends CI_Controller {
    ...
    public function index() {
            ...
            $data['jenis_uang_makan'] = $this->perkiraan->GetDataPerkiraanByStatus(3);
            ...
            $this->load->view('admin/pengaturanUmakan/pengaturanUmakan_view',$data);
    }

    public function tambahUangMakan(){
        $perkiraan_id = $this->input->post('hidden_perkiraan_id');
        $jumlah_penghasilan = $this->input->post('hidden_jumlah_uang_makan');
    ...
    public function editUangMakan($id_rtg){
        $data['title'] = "Admin Page | Sigawai";
        ...
        $this->load->view('templates/admin_topbar',$data);
        $this->load->view('templates/admin_sidebar',$data);
        $this->load->view('admin/pengaturanUmakan/editUangMakan_view',$data);
    }

    public function doEditUangMakan() {
        ...
        if($result >= 1) {
                ...
                if($result >= 1) {
                    ...
                    '<div class="alert alert-dismissible alert-success">
                            ...
                            Data Uang Makan Telah Diupdate!
                    </div>');
                    redirect('Umakan/referensi');
                } else {
                    $this->session->set_flashdata('message', 
                    '<div class="alert alert-dismissible alert-danger">
                        <button type="button" class="close" data-dismiss="alert">&times;</button>
                        Data Uang Makan Gagal Diupdate
                    </div>');
                    redirect('Umakan/editUangMakan/'.$id_rtg);
                }
    ...
    public function hapusUangMakan($id_rtg) 
    {
        $this->load->model('gaji_model','gaji');
        ...
        if($result >= 1) {
            ...
            $this->session->set_flashdata("message","<div class='alert alert-success' role='alert'>Data Uang Makan Berhasil Dihapus.</div>");
            redirect('Umakan/referensi');
        } else {
            $this->session->set_flashdata('message', 
            '<div class="alert alert-dismissible alert-danger">
                  <button type="button" class="close" data-dismiss="alert">&times;</button>
                  Data Uang Makan Gagal Di Delete
            </div>');
            redirect('Umakan/referensi');
        }
